...
export default function StrategiesPage() {
  ...
  const loadStrategy = async (name: string) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/strategies/${name}`);
      const data = await res.json();
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await fetch(`http://localhost:8000/strategies/${selectedStrategy}`, {
        method: "PUT",
      ...
      <div className="flex-1 flex flex-col">
        <div className="h-14 border-b border-slate-800 bg-slate-900/80 flex items-center justify-between px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              className="flex items-center gap-2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm font-medium rounded-md transition-colors"
            >
